...
# Changes
# -------
# Installed Slugify Python library which will strip out foreign characters from a track, artist or album name. This should fix a bug where foreign characters will cause an error when downloading all songs in a playlist.
# Updated to latest gmusicapi which supports OAuth and supporrts Python 3
# Updated to Python 3.7
# Updated PySide to PySide2
# Added OAuth support for MobileClient interface so you do not need to provide an app password any more
# Removed code that made up login screen since you have to log in now using OAuth
# Removed username and password variables
# CSV is now written using built in Python CSV writer for better reliability
# const Google OAuth cred filename is used now
# Rewrote command line usage text so it is formatted in an easier to read way
# Replaced newlines with os.linesep to better support Mac and Linux
# Fixed issue with findDuplicates command line option
# Fixed issue with command line argument case sensitivity for Dropbox where if you specify DropBox, instead of Dropbox the command line argument wouldn't be interpreted correctly
# If invalid command line argument is given, the command line usage for that specific parameter is shown instead of the help for all command line arguments
# When invalid arguments are given, the # of parameters expected and given are added to the error output for better error handling
# Created batch file to help do automated unit testing using the command line parameters 
# Fixed broken feature Create playlist from CSV
# Updated Dropbox to use OAuth2 and updated code to reflect API changes
# Fixed downloading all songs in a playlist BUT it requires you to do OAuth for a 2nd time
# CSV header was not consistent; Sometimes it wrote the ID, other times it didn't. Uses fixed now
# Fixed m3u creation so the m3u file is now correctly generated per directory
# Fixed issued with messagebox being shown when running command from terminal instead of printing to terminal the way its supposed to
# Fixed issue with SSL failure when running application as Windows binary by providing cacert.pem
# Improved Google OAuth error logging for easier debugging
# Fixed bug that asked for DropBox login on every login. It is now done only when needed
...
class GMusicUtility(QWidget):
     ...
     def __init__(self):
          ...
          # self.aesk = "dtDTzza?*v]aadmhf3xRVr5j)qkz7err"
          ...
          # self.aes = pyaes.AESModeOfOperationCTR(self.aesk.encode())
		  
          # ciphertext = self.aes.encrypt('lRzJ9ydr7MkAAAAAAAB3ZZ1411lnMIx621WgiJCpPFQfwVNqIhCtKADXhTRanmcr')
          # print(ciphertext)
     ...
     def commandLineUsage(self,param=None):
          ...
          if param is None or param == "/exportlibrary":
               ...
               print (chr(9) + chr(9) + "Syntax: " + sys.argv[0] + " /exportlibrary path format where format is CSV or HTML")
               ...
               for root, dirnames, filenames in os.walk(dir):
                    for file in filenames:
                         ...
                         if file.endswith('.mp3'):
                              ...
                              # (root if fullPath else root.replace(dir + self.delimiter, '')
                              '''meta_info = {
                                   'filename': root.replace(dir + self.delimiter,"") + self.delimiter + file,
                                   'length': int(MP3(root + self.delimiter + file).info.length),
                                   'tracknumber': EasyID3(root + self.delimiter + file)['tracknumber'][0].split('/')[0],
                                   'artist': EasyID3(root + self.delimiter + file)['artist'][0],
                                   'title': EasyID3(root + self.delimiter + file)['title'][0]
                              }
                    ...
                    '''if len(mp3s) > 0:
                         ...
                         with open(playlist, 'w') as out:
                              ...
                              for mp3 in sorted(mp3s, key=lambda mp3: int(mp3['tracknumber'])):
                                   out.write("#EXTINF: " + str(mp3['length']) + "," + mp3['artist'] + " - " + mp3['title'] + os.linesep)
                                   out.write("#EXTM3U" + os.linesep)
                    else:
               ...
               if playlist["name"] == playlistName:
                    for track in playlist["tracks"]:
                         ...
                         if saveLocation == "Locally":
                              ...
                              with open(downloadPath + self.delimiter + filename, 'wb') as f:
                                   # Write the audio buffer
                                   # print "Downloading " + downloadPath + self.delimiter + filename
                                   f.write(audio)
                                   f.close()

                                   if noSubDirectories == False:
                                        artistFolder=self.sanitizeString(downloadPath + self.delimiter + self.library[track['trackId']][3])
                                   else:
                                        artistFolder=self.sanitizeString(downloadPath)
                                   ...
                                   if noSubDirectories == False:
                                        albumFolder=self.sanitizeString(artistFolder + self.delimiter + self.library[track['trackId']][2])
                                   else:
                                        albumFolder=self.sanitizeString(artistFolder)
                                   ...
                                   trackFile=self.sanitizeString(albumFolder + self.delimiter + filename)
                              ...
                              if noSubDirectories == False:
                                   artistFolder=self.sanitizeString("/" + playlistName + "/" + self.library[track['trackId']][3])
                              else:
                                   artistFolder=self.sanitizeString("/" + playlistName)
                                   
                              try:
                                   # Create the folder uncoditionally because if it existed before and was deleted, we need to recreate it
                                   try:
                                        self.dropBoxClient.file_create_folder(artistFolder)
                                   except dbrest.ErrorResponse as e:
                                   ...
                                   """
                                   folder_metadata = self.dropBoxClient.metadata(artistFolder, include_deleted=False)
                              ...
                              if noSubDirectories == False:
                                   albumFolder=self.sanitizeString(artistFolder + "/" + self.library[track['trackId']][2])
                              else:
                                   albumFolder=self.sanitizeString(artistFolder)
                              ...
                              with open(filename, 'rb') as output:
                                   ...
                                   trackFile=self.sanitizeString(albumFolder + "/" + filename)
                                   
                                   # Move the file to Artist/Album folder
                                   #if noSubDirectories == False:
                                   response = self.dropBoxClient.file_move(filename,trackFile)
                                   #else:
                                   #     response = self.dropBoxClient.file_move(filename,trackFile)
                                   output.close()
          ...
          if exportFormat == "CSV":
               ...
               with open(fileName, 'w') as f:
                    ...
                    for num in range(0, len(library)-1):
                         row=[library[num]["id"],library[num]["title"],library[num]["album"],library[num]["artist"],library[num]["trackNumber"],library[num]["year"],library[num]["albumArtist"],library[num]["discNumber"],library[num]["genre"]];
                         writer.writerow(row);
          elif exportFormat == "HTML":
               with open(fileName, 'w') as out:
                    ...
                    for num in range(0, len(library)-1):				 
                         row="<TR><TD>" + library[num]["id"] + "</TD><TD>" + library[num]["title"] + "</TD><TD>" + library[num]["artist"] + "</TD><TD>" + library[num]["album"] + "</TD><TD>" + str(library[num]["year"]) + "</TD><TD>" + str(library[num]["albumArtist"]) + "</TD><TD>" + str(library[num]["trackNumber"]) + "</TD><TD>" + str(library[num]["discNumber"]) + "</TD><TD>" + str(library[num]["genre"]) + "</TD><TD></TD></TR>" + os.linesep
          ...
          if exportFormat == "CSV":
               ...
               with open(fileName, 'w') as f:
                    ...
                    for playlisttrack in playlisttracks:
                         ...
                         writer.writerow(playlisttrack)
          elif exportFormat == "HTML":
               with open(fileName, 'w') as out:
                    out.write(self.HTMLHeader)
     
                    for playlisttrack in playlisttracks:				 
                         row="<TR><TD>" + playlisttrack[0] + "</TD><TD>" + playlisttrack[1] + "</TD><TD>" + playlisttrack[2] + "</TD><TD>" + playlisttrack[3] + "</TD><TD>" + str(playlisttrack[4]) + "</TD><TD>" + str(playlisttrack[5]) + "</TD><TD>" + str(playlisttrack[6]) + "</TD><TD>" + str(playlisttrack[7]) + "</TD><TD>" + str(playlisttrack[8]) + "</TD></TR>" + os.linesep
          ...
          if exportFormat == "CSV":
               ...
               with open(fileName, 'w') as f:
                    ...
                    for song in self.newSongs:
                         row=[song[0],song[1],song[2],song[3],song[4],song[5],song[6],song[7],song[8]]
               ...
               with open(fileName, 'w') as out:
                    ...
                    for song in self.newSongs:				 
                         row="<TR><TD>" + song[0] + "</TD><TD>" + song[1] + "</TD><TD>" + song[2] + "</TD><TD>" + song[3] + "</TD><TD>" + str(song[4]) + "</TD><TD>" + str(song[5]) + "</TD><TD>" + str(song[6]) + "</TD><TD>" + str(song[7]) + "</TD><TD>" + str(song[8]) + "</TD>" + os.linesep
     ...
     def findDuplicateTracksInPlaylist(self,playlist1,playlist2,isCommandLine=False, saveToDropbox=False, fileName=None):
          ...
          with open(fileName, 'w') as f:
               ...
               for id in dupes:
                    row=[self.library[id][0],self.library[id][1],self.library[id][2],self.library[id][3],self.library[id][4],self.library[id][5],self.library[id][6],self.library[id][7],self.library[id][8]];
                    writer.writerow(row);
          ...
          for key in claParameters:
               if key == sys.argv[1]:
                    ...
                    if len(sys.argv)-1 != claParameters[key]:
                         print("Invalid number of command line parameter(s) given for " + sys.argv[1] + ". Expected " + str(claParameters[key]) + " parameters but received " + str(len(sys.argv)-1) + " parameters")
     ...
     def performDropboxOAuth(self):
          ...
          #self.aesk = aes = pyaes.AESModeOfOperationCTR(self.aesk)

          dbx = dropbox.Dropbox('lRzJ9ydr7MkAAAAAAAB3ZZ1411lnMIx621WgiJCpPFQfwVNqIhCtKADXhTRanmcr')
          ...
          if os.path.isfile(dropBoxFilename) == False:
               flow = DropboxOAuth2FlowNoRedirect("5unm4o2obvti68o","2ogpieix6doo8nu")
     ...
     # String cleanser
     def sanitizeString(self,str):
          badChars = ['/','\\',':','*','?','"','<','>','|']
          
          for currChar in badChars:
               str.replace(currChar,'_')
          
          return str #return slugify(str)
